...
// MocaClient holds all necessary information for creating/querying transactions.
...
// NewMocaClient is used to create a new MocaClient structure.
func NewMocaClient(rpcAddr, evmRpcAddr, chainID string, opts ...MocaClientOption) (*MocaClient, error) {
...
func NewCustomMocaClient(rpcAddr, evmRpcAddr, chainID string, customDialer func(string) (*http.Client, error), opts ...MocaClientOption) (*MocaClient, error) {
...
func setClientsConn(c *MocaClient, conn grpc1.ClientConn, evmCli *ethclient.Client) {
...
// SetKeyManager sets a key manager in the MocaClient structure.
func (c *MocaClient) SetKeyManager(keyManager keys.KeyManager) {
	c.keyManager = keyManager
}

// GetKeyManager returns the key manager set in the MocaClient structure.
func (c *MocaClient) GetKeyManager() (keys.KeyManager, error) {
	if c.keyManager == nil {
...
// SetChainId sets the chain ID in the MocaClient structure.
func (c *MocaClient) SetChainId(id string) { //nolint
	c.chainID = id
}

// GetChainID returns the chain ID set in the MocaClient structure.
func (c *MocaClient) GetChainID() (string, error) {
	if c.chainID == "" {
...
func (c *MocaClient) GetCodec() *codec.ProtoCodec {
	return c.codec
}
