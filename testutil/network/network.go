...
func DefaultConfig() Config {
	chainID := fmt.Sprintf("moca_%d-1", cmtrand.Int63n(9999999999999)+1)
...
func New(l Logger, baseDir string, cfg Config) (*Network, error) {
	...
	for i := 0; i < cfg.NumValidators; i++ {
		...
		nodeDir := filepath.Join(network.BaseDir, nodeDirName, "mocad")
		clientDir := filepath.Join(network.BaseDir, nodeDirName, "mocacli")
