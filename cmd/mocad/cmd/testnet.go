...
func addTestnetFlagsToCmd(cmd *cobra.Command) {
	...
	cmd.Flags().String(flags.FlagChainID, "", "genesis file chain-id, if left blank will default to moca_5151-1")
...
Example:
	mocad testnet init-files --v 4 --output-dir ./.testnets --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "mocad", "Home directory of the node's daemon configuration")
...
Example:
	mocad testnet --v 4 --output-dir ./.testnets
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			args := startArgs{}
			args.outputDir, _ = cmd.Flags().GetString(flagOutputDir)
...
func initTestnetFiles(
	...
	for i := 0; i < args.numValidators; i++ {
		...
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			...
			stakingtypes.NewCommissionRates(math.LegacyOneDec().QuoInt64(10), math.LegacyOneDec(), math.LegacyOneDec()),
			math.OneInt(),
			addr,
			addr,
			addr,
